...
def append_to_results_file(path, data_to_append):
	...
	results_file.writelines(str(data_to_append).lstrip("(' \n ").rstrip("')") + "\n")
	results_file.close()

def try_telnet(ip):
	host_telnet_object = telnetlib.Telnet()
	try:
		host_telnet_object.open(ip, 23, timeout=1)
...
# and try another username and password within
	...
	if old_session == '':
		try:
			session = host_telnet_object.read_until(b"Username: ", timeout=1)
		except Exception as e:
			return(str(e))
	else:
		session = old_session
...
#####################
	if authenticated and privmode:
		...
		sw_ip = ("Switch ip = " + switch_ip)
	# Get hostname	
		current_session_output = telnet_send_command(current_host, 'sh mac address-table | exclude Po')
		print(((sw_ip) + "\n" + (current_session_output.decode('ascii'))))
		hostname_got = ((sw_ip) + "\n" + (current_session_output.decode('ascii')))
